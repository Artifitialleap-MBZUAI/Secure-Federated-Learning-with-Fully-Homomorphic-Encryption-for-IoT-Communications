...
def create_encrypted_model_dic(number_of_clusters):
    ...
    return encrypted_model_dict, decrypted_model_dict

def Net2nn_Enc(num_of_users):
  fc1_enc_ma = torch.zeros(size=[10, num_of_features])
  fc2_enc_ma = torch.zeros(size=[10, 10])
  fc3_enc_ma = torch.zeros(size=[2 , 10])
  return fc1_enc_ma, fc2_enc_ma, fc3_enc_ma

def create_Enc_matrix_for_clusters(groups):

  matrix_dict_Enc = {"cluster_"+str(i): {} for i in range(number_of_clusters)}
  for i in range(len(groups)):
    for j in range(3):
        matrix_name_enc = "fc_"+str(j)
        matrix_info_enc = Net2nn_Enc()
        matrix_dict_Enc["cluster_"+str(i)].update({matrix_name_enc : matrix_info_enc})

  return matrix_dict_Enc
...
def enc_model(B_matrix_update_, encrypted_model_dict, groups):
  with torch.no_grad():
      ...
      for i in range(len(groups)):
        ...
        for j in (groups[i]):
          ...
          matrix_fc1_W += B_matrix_update_[i]['user_'+str(j)].fc1.weight

          matrix_fc2_W += B_matrix_update_[i]['user_'+str(j)].fc2.weight

          matrix_fc3_W += B_matrix_update_[i]['user_'+str(j)].fc3.weight
          
          matrix_fc1_B += B_matrix_update_[i]['user_'+str(j)].fc1.bias
          cluster_size += sys.getsizeof(B_matrix_update_[i]['user_'+str(j)].fc1.bias)
          matrix_fc2_B += B_matrix_update_[i]['user_'+str(j)].fc2.bias
          cluster_size += sys.getsizeof(B_matrix_update_[i]['user_'+str(j)].fc2.bias)
          matrix_fc3_B += B_matrix_update_[i]['user_'+str(j)].fc3.bias
          cluster_size += sys.getsizeof(B_matrix_update_[i]['user_'+str(j)].fc3.bias)

        E_W1 = apply_FHE(matrix_fc1_W)  
        E_W2 = apply_FHE(matrix_fc2_W) 
        E_W3 = apply_FHE(matrix_fc3_W) 
        
        clus_size = sys.getsizeof(E_W1) + sys.getsizeof(E_W2) + sys.getsizeof(E_W3) + cluster_size
        ...
        encrypted_model_dict[name_of_enc_models[i]].fc1_enc_B1 = matrix_fc1_B
        encrypted_model_dict[name_of_enc_models[i]].fc2_enc_B2 = matrix_fc2_B
        encrypted_model_dict[name_of_enc_models[i]].fc3_enc_B3 = matrix_fc3_B


  return encrypted_model_dict , clus_size

def create_matrix_for_clusters(groups):

  matrix_dict_ = {"Cluster_"+str(i): {} for i in range(number_of_clusters)}
  for i in range(len(groups)):
    for j in (groups[i]):
        matrix_name_user = "user_"+str(j)
        matrix_info_user = Net2nn()
        matrix_dict_["Cluster_"+str(i)].update( {matrix_name_user : matrix_info_user})

  return matrix_dict_

def dec_model(encrypted_model_dict, decrypted_model_dict, number_of_clusters ):
...
def Server_get_averaged_weights(group_list_Enc,groups, number_of_samples):

  fc1_mean_weight = torch.zeros(size=group_list_Enc[name_of_enc_models[0]].fc1_enc.shape)
  fc2_mean_weight = torch.zeros(size=group_list_Enc[name_of_enc_models[0]].fc2_enc.shape)
  fc3_mean_weight = torch.zeros(size=group_list_Enc[name_of_enc_models[0]].fc3_enc.shape)

  fc1_mean_bias = torch.zeros(size=group_list_Enc[name_of_enc_models[0]].fc1_enc_B1.shape)
  fc2_mean_bias = torch.zeros(size=group_list_Enc[name_of_enc_models[0]].fc2_enc_B2.shape)
  fc3_mean_bias = torch.zeros(size=group_list_Enc[name_of_enc_models[0]].fc3_enc_B3.shape)

  
  total_size_Enc_W_B = 0
  for j in range(len(groups)):        
  
    fc1_mean_weight += group_list_Enc[name_of_enc_models[j]].fc1_enc
    total_size_Enc_W_B += sys.getsizeof(group_list_Enc[name_of_enc_models[j]].fc1_enc)

    fc2_mean_weight += group_list_Enc[name_of_enc_models[j]].fc2_enc
    total_size_Enc_W_B += sys.getsizeof(group_list_Enc[name_of_enc_models[j]].fc2_enc)

    fc3_mean_weight += group_list_Enc[name_of_enc_models[j]].fc3_enc
    total_size_Enc_W_B += sys.getsizeof(group_list_Enc[name_of_enc_models[j]].fc3_enc)

    fc1_mean_bias += group_list_Enc[name_of_enc_models[j]].fc1_enc_B1 
    total_size_Enc_W_B += sys.getsizeof(group_list_Enc[name_of_enc_models[j]].fc1_enc_B1 )

    fc2_mean_bias += group_list_Enc[name_of_enc_models[j]].fc2_enc_B2
    total_size_Enc_W_B += sys.getsizeof(group_list_Enc[name_of_enc_models[j]].fc2_enc_B2 )
 
    fc3_mean_bias += group_list_Enc[name_of_enc_models[j]].fc3_enc_B3
    total_size_Enc_W_B += sys.getsizeof(group_list_Enc[name_of_enc_models[j]].fc3_enc_B3 )

    total_size_Enc_W_B_C0 = total_size_Enc_W_B


  plain_number_of_samples = ts.plain_tensor(1/number_of_samples)
  fc1_mean_weight =fc1_mean_weight * plain_number_of_samples
  fc1_mean_bias = fc1_mean_bias/ number_of_samples

  fc2_mean_weight =fc2_mean_weight * plain_number_of_samples
  fc2_mean_bias = fc2_mean_bias/ number_of_samples

  fc3_mean_weight =fc3_mean_weight * plain_number_of_samples    
  fc3_mean_bias = fc3_mean_bias/ number_of_samples

  return fc1_mean_weight, fc1_mean_bias, fc2_mean_weight, fc2_mean_bias, fc3_mean_weight, fc3_mean_bias, total_size_Enc_W_B_C0

def Batching_Matrix_Update(matrix_dict):
  matrix_update = []
  for i in range (len(matrix_dict)):
    matrix_update.append(matrix_dict['Cluster_'+str(i)])
  #print(server_update)

  return matrix_update

def packed_tensors_and_Encrypt(matrix_dict_, groups):

  list_group = [[] for _ in range(len(groups))]
  ...
  for i in range(len(groups)):
    ...
    for j in ((groups[i])):
      ...
      tensors_fc1.append(matrix_dict_["Cluster_"+str(i)]["user_"+str(j)].fc1.weight.data)
      tensors_fc2.append(matrix_dict_["Cluster_"+str(i)]["user_"+str(j)].fc2.weight.data)
      tensors_fc3.append(matrix_dict_["Cluster_"+str(i)]["user_"+str(j)].fc3.weight.data)


    list_group[i].append(torch.cat(tensors_fc1, dim=0))
    list_group[i].append(torch.cat(tensors_fc2, dim=0))
    list_group[i].append(torch.cat(tensors_fc3, dim=0))


  return list_group

def update_dec_main_model(dec_main_model, decrypted_model_dict):
    with torch.no_grad():
...
def send_Enc_model_to_nodes_and_update_Enc_model_dict(enc_main_model, encrypted_model_dict, number_of_clusters):
    with torch.no_grad():
        for i in range(number_of_clusters):
...
def distribute_users_into_clusters(number_of_clusters, number_of_samples):

    users = list(range(number_of_samples))

    random.shuffle(users)

    group_size = number_of_samples // number_of_clusters

    groups = [[] for _ in range(number_of_clusters)]

    for i, user in enumerate(users):
        group_index = i // group_size
        if group_index == number_of_clusters:
            group_index -= 1
...
def send_model_to_Batching_Matrix_updates(matrix_dict_, model_dict, number_of_clusters, number_of_samples, groups):
    with torch.no_grad():
      
      for i in range(len(groups)):
        for j in ((groups[i])):

          matrix_dict_["Cluster_"+str(i)]["user_"+str(j)].fc1.weight.data = model_dict[name_of_models[j]].fc1.weight.data
          matrix_dict_["Cluster_"+str(i)]["user_"+str(j)].fc2.weight.data = model_dict[name_of_models[j]].fc2.weight.data
          matrix_dict_["Cluster_"+str(i)]["user_"+str(j)].fc3.weight.data = model_dict[name_of_models[j]].fc3.weight.data

          matrix_dict_["Cluster_"+str(i)]["user_"+str(j)].fc1.bias.data = model_dict[name_of_models[j]].fc1.bias.data
          matrix_dict_["Cluster_"+str(i)]["user_"+str(j)].fc2.bias.data = model_dict[name_of_models[j]].fc2.bias.data
          matrix_dict_["Cluster_"+str(i)]["user_"+str(j)].fc3.bias.data = model_dict[name_of_models[j]].fc3.bias.data
...
def tensor_loader_and_data_loader(x_train, y_train, x_valid, y_valid,x_test, y_test):
...
train_dl, valid_dl, test_dl = tensor_loader_and_data_loader(x_train, y_train, x_valid, y_valid,x_test, y_test)
...
encrypted_model_dict, decrypted_model_dict = create_encrypted_model_dic(number_of_clusters)
...
#################################### Approach No.3: Decentralized Matrix without Encryption ####################################
ExportToFile= 'Appr3_Output'+dataset_name
...
for i in range(numEpoch):
    ...
    groups = distribute_users_into_clusters(number_of_clusters, number_of_samples)

    model_dict = start_train_end_node_process_without_print(number_of_samples, model_dict)

    matrix_dict_ = create_matrix_for_clusters(groups)

    matrix_dict_ = send_model_to_Batching_Matrix_updates(matrix_dict_, model_dict, number_of_clusters, number_of_samples, groups)

    B_matrix_update = Batching_Matrix_Update(matrix_dict_)

    encrypted_model_dict, cluster_size = enc_model(B_matrix_update, encrypted_model_dict,groups ) 

    fc1_mean_weight, fc1_mean_bias, fc2_mean_weight, fc2_mean_bias, fc3_mean_weight, fc3_mean_bias, total_size_Enc_W_B = Server_get_averaged_weights(encrypted_model_dict,groups, number_of_samples)
    ...
    encrypted_model_dict = send_Enc_model_to_nodes_and_update_Enc_model_dict(enc_main_model, encrypted_model_dict, number_of_clusters)

    decrypted_model_dict = dec_model(encrypted_model_dict, decrypted_model_dict, number_of_clusters)

    dec_main_model= update_dec_main_model(dec_main_model, decrypted_model_dict) 
    ...
    comm_overhead_B = total_size_Enc_W_B*number_of_clusters
    ...
    print("The comm_overhead: ", comm_overhead)
