...
def apply_FHE(v1):
    ...
    return encrypted_tensor, size_of_encrypted_tensor
...
def enc_model(model_dict, encrypted_model_dict, number_of_samples):
  with torch.no_grad():
      ...
      for i in range(number_of_samples):
        ...
        E_W1, Enc_size1 = apply_FHE(model_c1)       
        ...
        E_W2, Enc_size2 = apply_FHE(model_c2)
        ...
        E_W3, Enc_size3 = apply_FHE(model_c3)
...
def Server_get_averaged_weights(matrix_dict):   
    ...
    with torch.no_grad():
        for i in range(len(matrix_dict)):
          ...
          for j in range(len(matrix_dict[i])):
            user_id = groups[i][j]

            fc1_mean_weight += matrix_dict[i]['user_'+str(user_id)].fc1_enc
            total_size_Enc_W_B += sys.getsizeof(matrix_dict[i]['user_'+str(user_id)].fc1_enc)

            fc2_mean_weight += matrix_dict[i]['user_'+str(user_id)].fc2_enc
            total_size_Enc_W_B += sys.getsizeof(matrix_dict[i]['user_'+str(user_id)].fc2_enc)

            fc3_mean_weight += matrix_dict[i]['user_'+str(user_id)].fc3_enc   
            total_size_Enc_W_B += sys.getsizeof(matrix_dict[i]['user_'+str(user_id)].fc3_enc)

            fc1_mean_bias += matrix_dict[i]['user_'+str(user_id)].fc1_enc_B1 
            total_size_Enc_W_B += sys.getsizeof(matrix_dict[i]['user_'+str(user_id)].fc1_enc_B1)

            fc2_mean_bias += matrix_dict[i]['user_'+str(user_id)].fc2_enc_B2 
            total_size_Enc_W_B += sys.getsizeof(matrix_dict[i]['user_'+str(user_id)].fc2_enc_B2)

            fc3_mean_bias += matrix_dict[i]['user_'+str(user_id)].fc3_enc_B3 
            total_size_Enc_W_B += sys.getsizeof(matrix_dict[i]['user_'+str(user_id)].fc3_enc_B3)
            
            total_size.append(total_size_Enc_W_B)
...
def copy_dec_main_model_to_main_model(main_model, dec_main_model):
    with torch.no_grad():
        ...
        main_model.fc3.bias.data = torch.tensor(dec_main_model.fc3_dec_B3)
...
def send_Enc_model_to_Batching_Matrix_updates(matrix_dict_, encrypted_model_dict, number_of_clusters, number_of_samples, groups):
    cluster_size=0
    with torch.no_grad():       
      for i in range(len(groups)):
        ...
        for j in ((groups[i])):
          ...
          matrix_dict_["Cluster_"+str(i)]["user_"+str(j)].fc1_enc = encrypted_model_dict[name_of_enc_models[j]].fc1_enc
          cluster_size += sys.getsizeof(encrypted_model_dict[name_of_enc_models[j]].fc1_enc)
          matrix_dict_["Cluster_"+str(i)]["user_"+str(j)].fc2_enc = encrypted_model_dict[name_of_enc_models[j]].fc2_enc
          cluster_size += sys.getsizeof(encrypted_model_dict[name_of_enc_models[j]].fc2_enc)
          matrix_dict_["Cluster_"+str(i)]["user_"+str(j)].fc3_enc = encrypted_model_dict[name_of_enc_models[j]].fc3_enc
          cluster_size += sys.getsizeof(encrypted_model_dict[name_of_enc_models[j]].fc3_enc)

          matrix_dict_["Cluster_"+str(i)]["user_"+str(j)].fc1_enc_B1 = encrypted_model_dict[name_of_enc_models[j]].fc1_enc_B1
          cluster_size += sys.getsizeof(encrypted_model_dict[name_of_enc_models[j]].fc1_enc_B1)
          matrix_dict_["Cluster_"+str(i)]["user_"+str(j)].fc2_enc_B2 = encrypted_model_dict[name_of_enc_models[j]].fc2_enc_B2
          cluster_size += sys.getsizeof(encrypted_model_dict[name_of_enc_models[j]].fc2_enc_B2)
          matrix_dict_["Cluster_"+str(i)]["user_"+str(j)].fc3_enc_B3 = encrypted_model_dict[name_of_enc_models[j]].fc3_enc_B3
          cluster_size += sys.getsizeof(encrypted_model_dict[name_of_enc_models[j]].fc3_enc_B3)
  
    return matrix_dict_, cluster_size
...
#################################### Approach No.4: Decentralized Matrix with Encryption ####################################
ExportToFile= 'Appr4_Output'+dataset_name
...
for i in range(numEpoch):
    ...
    matrix_dict_, cluster_size = send_Enc_model_to_Batching_Matrix_updates(matrix_dict_, encrypted_model_dict, number_of_clusters, number_of_samples, groups)

    B_matrix_update = Batching_Matrix_Update(matrix_dict_)

    fc1_mean_weight, fc1_mean_bias, fc2_mean_weight, fc2_mean_bias, fc3_mean_weight, fc3_mean_bias, total_size_Enc_W_B = Server_get_averaged_weights(B_matrix_update)
    ...
    comm_overhead_B = cluster_size*number_of_clusters
